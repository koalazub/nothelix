...
pub(super) struct PlaceholderGrid {
    cols: usize,
    rows: usize,
}

impl PlaceholderGrid {
    pub(super) fn new(cols: usize, rows: usize) -> Option<Self> {
        (cols <= capacity() && rows <= capacity()).then_some(Self { cols, rows })
    }

    pub(super) fn render(&self) -> String {
        let mut out = String::with_capacity(self.rows * (self.cols * 4 + 4 + 1));
        for (index, row_mark) in DIACRITICS.iter().enumerate().take(self.rows) {
            out.push(PLACEHOLDER);
            out.push(*row_mark);
            out.push(DIACRITICS[0]);
            for _ in 1..self.cols {
                out.push(PLACEHOLDER);
            }
